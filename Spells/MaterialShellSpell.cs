...
{
    internal class MaterialShellSpell : Spell
    {
        ...
        public MaterialShellSpell(string name, string incantation, int radius, ushort tileType) : base(name, incantation, SpellType.Spell)
        {
            _radius = radius;
            _tileType = tileType;
        }
        ...
        public int DoBlockBomb(Vector2 position, ushort tileType, int radius, int dustType, float dustFactor, int tileCount = -1)
        {
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = radius; y >= -radius; y--)
                {
                    ...
                    if ((double)(x * x + y * y) <= ((double)radius + 0.5) * ((double)radius + 0.5))
                    {
                        ...
                        if (!tile.HasTile)
                        {
                            ...
                            if (placed && Main.netMode == NetmodeID.MultiplayerClient)
                                NetMessage.SendData(MessageID.TileManipulation, -1, -1, null, 1, (float)xPos, (float)yPos, (float)tileType, 0);
                        }
